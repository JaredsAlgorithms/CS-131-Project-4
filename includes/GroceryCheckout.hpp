...
struct GroceryItem {
  GroceryItem() {}
  GroceryItem(const std::string& name, int quantity, float price, bool taxable)
...
struct ReceiptItem {
  ReceiptItem() {}
  ReceiptItem(const std::string& name, float price)
      : name_(name), price_(price) {}
  ReceiptItem(const GroceryItem& item)
      : name_(item.name_), price_(item.price_) {}
  std::string name_;
  float price_;
};
...
class GroceryInventory {  // defined in GroceryCheckout.cpp
 public:
  bool AddItem(const std::string& name, int quantity, float price,
	       bool taxable);
