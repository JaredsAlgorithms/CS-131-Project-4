...
//********************************************************************************
float Difference(float f1, float f2) { return (fabs(f1 - f2)); }
...
uint64_t TestAddItem() {
  ...
  success = inventory.AddItem("two", 1, 1.0, 0);
  if (!success) {
    cout << "PASSED: GroceryInventory::AddItem, duplicate detected as "
	    "expected, 8 points"
	 << endl;
    score += 8;
  } else {
    cout << "FAILED: GroceryInventory::AddItem, duplicate not detected but "
	    "should have been."
	 << endl;
  }
...
//********************************************************************************
uint64_t TestCreateInventory(GroceryInventory& inventory) {
  ifstream file("inputs/shipment.txt");
  GroceryItem* itemPtr;
  string name;
  bool pass;
  float price;
  ...
  inventory.CreateFromFile("inputs/shipment.txt");
  ...
  while (true) {
    ...
    if (itemPtr == nullptr) {
      cout << "FAILED: GroceryInventory::CreateFromFile missing item: " << name
	   << endl;
      pass = false;
      break;
    }
    if (itemPtr->quantity_ != quantity) {
      cout << "FAILED: GroceryInventory::CreateFromFile wrong quantity for "
	   << name << endl;
      pass = false;
      break;
    }
    if (itemPtr->price_ != price) {
      cout << "FAILED: GroceryInventory::CreateFromFile wrong price for" << name
	   << endl;
      pass = false;
      break;
    }
    if (itemPtr->taxable_ != taxable) {
      cout << "FAILED: GroceryInventory::CreateFromFile wrong taxable for"
	   << name << endl;
      pass = false;
      break;
    }
  ...
  if (pass) {
    if (size == inventory.Size()) {
      score = 12;
      cout << "PASSED: GroceryInventory::CreateFromFile actual matches "
	      "expected. "
	   << score << " points." << endl;
    } else {
      cout << "FAILED: GroceryInventory::CreateFromFile actual size doesn't "
	      "match expected."
	   << endl;
    }
  }
...
uint64_t TestCreateReceipt(GroceryInventory& inventory) {
  ...
  actualReceipt = inventory.CreateReceipt("inputs/checkout.txt");
  ...
  if (success) {
    score = 15;
    cout << endl
	 << "PASSED: Receipt::CreateFromFile, actual matches expected, "
	 << score << " points." << endl;
    finalScore += score;
  } else {
    cout << endl
	 << "FAILED: Receipt::CreateFromFile, actual doesn't match expected."
	 << endl;
  }
  ...
  if (success) {
    score = 5;
    cout << endl
	 << "PASSED: Receipt::CreateFromFile, actual inventory quantities "
	    "match expected, "
	 << score << " points." << endl;
    finalScore += score;
  } else {
    cout << endl
	 << "FAILED: Receipt::CreateFromFile, actual inventory quantities "
...
uint64_t TestFindItem() {
  ...
  if (itemPtr != nullptr) {
    cout << "PASSED: GroceryInventory::FindItem, existing item found, 8 points"
	 << endl;
    score += 8;
  } else {
    cout << "FAILED: GroceryInventory::FindItem, existing item not found."
	 << endl;
  }

  itemPtr = inventory.FindItem("four");
  if (itemPtr == nullptr) {
    cout << "PASSED: GroceryInventory::FindItem, nonexistent item not found, 8 "
	    "points"
	 << endl;
    score += 8;
  } else {
    cout << "FAILED: GroceryInventory::FindItem, nonexistent item found but "
	    "shouldn't have been."
	 << endl;
  }
...
uint64_t TestRemoveItem() {
  ...
  if (success) {
    itemPtr = inventory.FindItem("two");
    if (itemPtr == nullptr) {
      cout << "PASSED: GroceryInventory::RemoveItem, existing item removed, 8 "
	      "points"
	   << endl;
      score += 8;
    } else {
  ...
  if (!success) {
    cout << "PASSED: GroceryInventory::RemoveItem, nonexistent item not "
	    "removed, 8 points"
	 << endl;
    score += 8;
  } else {
    cout << "FAILED: GroceryInventory::RemoveItem, nonexistent item removed "
...
//********************************************************************************
bool VerifyReceipt(Receipt& actualReceipt) {
  vector<ReceiptItem> item = {{"Salad_Dressing", 1.39},
			      {"Oat_Meal", 1.49},
